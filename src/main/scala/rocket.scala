...
class Rocket(id:Int)(implicit p: Parameters) extends CoreModule()(p) {
  ...
  val mem_int_wtag  = Mux(mem_ctrl.jalr||mem_ctrl.jal, csr.io.tag_ctrl.maskJmpProp,
                                                       mem_reg_wtag)
  ...
  val (mem_xcpt, mem_cause) = checkExceptions(List(
    ...
    (mem_reg_valid && mem_pc_tag_xcpt,                    UInt(Causes.tag_check_failure)),
  ...
  when (mem_reg_valid || mem_reg_replay || mem_reg_xcpt_interrupt) {
    ...
    wb_reg_wtag  := Mux(mem_ctrl.fp && mem_ctrl.wxd, UInt(0), mem_int_wtag)
    when (mem_ctrl.rocc) {
